...
- 같은 1배 데이터를 새로 적재하고 캐시를 워밍업한 뒤 요청량을 높이자 `250 rps`는 3회 모두 통과했고, `275 rps`는 가운데 값 기준으로 통과했지만 한 회차가 실패했으며, `300 rps`는 3회 모두 실패했다.
...
| 7 | [MID4-244 실패 경계 후속 재측정](mid4-244-rdb-failed-boundary-remeasure/README.md) | 부분 인덱스는 어디까지 효과가 있고 일반 조회는 300 rps보다 얼마나 더 처리할 수 있는가 | 데이터 몰림 1배는 275 rps가 변동이 있는 통과 지점이고 300 rps에서 반복 실패했다. 일반 데이터는 300 rps를 10분 유지했지만 350 rps는 10분 유지하지 못했다. 운영 인덱스 적용 여부는 결정하지 않았다. |
...
| MID4-244 | MID4-227과 MID4-206에서 실패한 두 조건부터 재현하고 부분 인덱스와 처리량 경계를 확인 | 일반 `300 rps · 10분`은 현재 코드로 통과했다. 일반 350 rps는 1분 3회에는 통과했지만 10분에는 실패했다. 실험용 인덱스의 1배 연결 데이터는 250 rps까지 3회 통과하고 275 rps는 가운데 값 기준 통과, 300 rps는 반복 실패했다. 2배 연결 데이터는 200 rps부터 실패했다. 후보 인덱스는 현재 배포 상태가 아니다. |
...
- 연결 데이터 1배는 `250 rps`까지 3회 통과, `275 rps`는 가운데 값 기준 통과지만 변동 있음, `300 rps`는 반복 실패로 기록한다. 연결 데이터 2배는 `200 rps`부터 실패했다.
