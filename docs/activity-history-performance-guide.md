...
| 7 | [MID4-244 실패 경계 후속 재측정](mid4-244-rdb-failed-boundary-remeasure/README.md) | 실패했던 조건부터 확인하면 부분 인덱스는 어디까지 효과가 있는가 | 실험용 인덱스로 1배 연결 데이터는 안정화했지만 2배 조건에서 최초 실패 경계를 확인했다. 운영 적용 여부는 결정하지 않았다. |
...
| MID4-244 | MID4-227과 MID4-206에서 실패한 두 조건부터 재현하고 부분 인덱스를 비교 | 이미 통과한 조건은 재측정하지 않았다. 일반 `300 rps · 10분`은 현재 코드로 통과했다. 실험용 인덱스의 1배 연결 데이터는 통과했지만 현재 배포 상태가 아니며, 2배 연결 데이터 `200 rps`가 최초 실패 지점이다. |
...
- 확인된 `200 rps · 연결 데이터 2배` 실패 지점을 현재 구조의 한계로 기록한다. 1배 결과도 로컬·단일 사용자·워밍업 캐시 조건이므로 운영 보장값으로 사용하지 않는다.
