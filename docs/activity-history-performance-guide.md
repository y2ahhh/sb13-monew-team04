...
- 실험용 인덱스를 사용한 데이터 몰림 기본 조건의 `200 rps · 10분`은 통과했지만, 요청 한 번에 약 `17.1만 개`의 인덱스 항목을 직접 세는 구조는 남았다.
- 같은 데이터 몰림 기본 조건을 새로 준비하고 캐시를 워밍업한 뒤 요청량을 높이자 `250 rps`는 3회 모두 통과했고, `275 rps`는 가운데 값 기준으로 통과했지만 한 회차가 실패했으며, `300 rps`는 3회 모두 실패했다.
- 한 대상에 몰린 데이터를 기본 조건의 2배로 늘린 `200 rps`에서는 확인 항목이 약 `34.1만 개`로 늘었고, 커버링 인덱스를 사용해도 첫 1분부터 응답 시간 기준을 넘었다.
- 현재 일반 데이터의 `300 rps · 10분`은 인덱스 후보 없이 통과했다. 이미 통과한 조건은 후보를 적용해 다시 측정하지 않았다.
- 일반 데이터에서 `300 rps`보다 높은 구간을 확인한 결과 `350 rps`는 1분 3회를 통과했지만 10분에는 실패했고, `375 rps`는 3회 중 1회, `400 rps`는 3회 중 2회가 실패했다. 따라서 현재 확인된 지속 통과 기준은 `300 rps`다.
...
| 7 | [MID4-244 실패 경계 후속 재측정](mid4-244-rdb-failed-boundary-remeasure/README.md) | 부분 인덱스는 어디까지 효과가 있고 일반 조회는 300 rps보다 얼마나 더 처리할 수 있는가 | 데이터 몰림 기본 조건은 275 rps가 변동이 있는 통과 지점이고 300 rps에서 반복 실패했다. 일반 데이터는 300 rps를 10분 유지했지만 350 rps는 10분 유지하지 못했다. 운영 인덱스 적용 여부는 결정하지 않았다. |
...
| MID4-244 | MID4-227과 MID4-206에서 실패한 두 조건부터 재현하고 부분 인덱스와 처리량 경계를 확인 | 일반 `300 rps · 10분`은 현재 코드로 통과했다. 일반 350 rps는 1분 3회에는 통과했지만 10분에는 실패했다. 실험용 인덱스를 적용한 데이터 몰림 기본 조건은 250 rps까지 3회 통과하고 275 rps는 가운데 값 기준 통과, 300 rps는 반복 실패했다. 데이터 몰림 2배 조건은 200 rps부터 실패했다. 후보 인덱스는 현재 배포 상태가 아니다. |
...
- 이번 작업에서는 조회 SQL을 별도로 변경하지 않는다. 요청마다 정확한 개수를 다시 세기 때문에 데이터 몰림 기본 조건에서도 약 `17.1만 개`, 2배 조건에서는 약 `34.1만 개`의 인덱스 항목을 확인했다.
- 데이터 몰림 기본 조건은 `250 rps`까지 3회 통과, `275 rps`는 가운데 값 기준 통과지만 변동 있음, `300 rps`는 반복 실패로 기록한다. 데이터 몰림 2배 조건은 `200 rps`부터 실패했다.
...
- MongoDB 조회용 데이터는 활동내역 조회 전체를 RDB와 분리해 독립적으로 늘려야 할 때 검토한다. 개수 계산만 문제라면 더 작은 변경인 카운터 테이블을 먼저 비교한다.
