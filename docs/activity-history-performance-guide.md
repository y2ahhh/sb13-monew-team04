...
- 활성 데이터만 담고 집계용 ID를 포함한 부분 커버링 인덱스는 테이블 재방문과 버퍼 사용량을 줄였다.
- 현재 일반 데이터의 `300 rps · 10분`은 통과했지만, 연결 데이터를 2배로 늘린 `200 rps`에서는 커버링 인덱스를 사용해도 응답 시간 기준을 넘었다.
...
| 7 | [MID4-244 실패 경계 후속 재측정](mid4-244-rdb-failed-boundary-remeasure/README.md) | 실패했던 조건부터 확인하면 부분 인덱스는 어디까지 효과가 있는가 | 1배 연결 데이터는 안정화했지만 2배 조건에서 최초 실패 경계를 확인했다. |
...
| MID4-244 | MID4-227과 MID4-206에서 실패한 두 조건부터 재현하고 부분 인덱스를 비교 | 이미 통과한 조건은 재측정하지 않았다. 일반 `300 rps · 10분`은 현재 코드로 통과했고, 2배 연결 데이터 `200 rps`가 최초 실패 지점이다. |
...
- 부분 커버링 인덱스 약 `479 MB`의 저장 공간과 쓰기 비용을 확인한 뒤 운영 DB 반영 여부와 적용 대상을 결정한다.
- 이번 작업에서는 조회 SQL을 별도로 변경하지 않으며, 확인된 `200 rps · 연결 데이터 2배` 실패 지점을 현재 구조의 한계로 기록한다.
