...
- 실험용 인덱스를 사용한 데이터 몰림 조건의 `200 rps · 10분`은 통과했지만, 요청 한 번에 약 `17.1만 개`의 인덱스 항목을 직접 세는 구조는 남았다.
...
- 같은 대상에 추가한 좋아요·댓글·조회·구독 데이터를 2배로 늘린 `200 rps`에서는 확인 항목이 약 `34.1만 개`로 늘었고, 커버링 인덱스를 사용해도 첫 1분부터 응답 시간 기준을 넘었다.
...
| 7 | [MID4-244 실패 경계 후속 재측정](mid4-244-rdb-failed-boundary-remeasure/README.md) | 부분 인덱스는 어디까지 효과가 있고 일반 조회는 300 rps보다 얼마나 더 처리할 수 있는가 | 데이터 몰림 조건은 275 rps가 변동이 있는 통과 지점이고 300 rps에서 반복 실패했다. 일반 데이터는 300 rps를 10분 유지했지만 350 rps는 10분 유지하지 못했다. 운영 인덱스 적용 여부는 결정하지 않았다. |
...
| MID4-244 | MID4-227과 MID4-206에서 실패한 두 조건부터 재현하고 부분 인덱스와 처리량 경계를 확인 | 일반 `300 rps · 10분`은 현재 코드로 통과했다. 일반 350 rps는 1분 3회에는 통과했지만 10분에는 실패했다. 실험용 인덱스를 적용한 데이터 몰림 조건은 250 rps까지 3회 통과하고 275 rps는 가운데 값 기준 통과, 300 rps는 반복 실패했다. 데이터 몰림 2배 조건은 200 rps부터 실패했다. 후보 인덱스는 현재 배포 상태가 아니다. |
