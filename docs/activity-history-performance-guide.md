# 활동 내역 조회 성능 개선 기록 안내서

## 이 문서가 답하는 질문

활동 내역 조회 성능과 관련된 여러 측정 문서를 어떤 순서로 읽어야 하는지 안내한다. 각 작업에서 무엇을 확인했고 다음 작업으로 왜 이어졌는지 먼저 설명한 뒤, 필요한 상세 측정값과 실행계획으로 이동할 수 있게 구성했다.

## 한눈에 보는 현재 결론

- 처음에는 데이터가 늘어날수록 사용자 활동을 찾는 SQL이 전체 테이블을 넓게 읽었지만, 사용자 조건과 최신순 조회에 맞는 인덱스를 추가하자 일반 데이터 조건의 SQL과 API가 크게 빨라졌다.
- 로컬 단기 측정에서는 `200 rps`, 더 긴 측정에서는 `190 rps`를 통과했다. 일반 조회만으로 MongoDB 조회 모델을 바로 도입할 근거는 부족해 적용을 후순위로 두었다.
- 한 댓글·기사·관심사에 데이터가 몰리거나 제외할 데이터가 많으면 반복 조회가 다시 느려졌고, 활동 데이터의 노출 상태를 직접 확인하도록 바꾸자 이 특수 조건도 개선됐다.
- 실험용 부분 커버링 인덱스 4개는 테이블 재방문과 버퍼 사용량을 줄였다. 기사 조회 수는 `COUNT(*)`를 사용하면 `INCLUDE(id)` 없는 `76 MB` 부분 인덱스도 Index Only Scan이 가능했다. 두 결과 모두 코드와 Flyway에는 반영하지 않았다.
- 후보 인덱스를 적용한 데이터 몰림 조건은 `250 rps`까지 3회 통과했고, `275 rps`는 변동 구간, `300 rps`는 반복 실패였다. 요청마다 약 `17.1만 개`의 인덱스 항목을 세는 구조는 남았다.
- 데이터 몰림 규모를 2배로 늘리자 요청당 확인 항목이 약 `34.1만 개`로 증가했고, 후보 인덱스를 사용해도 `200 rps`의 첫 1분부터 실패했다.
- 일반 데이터는 기존 인덱스로 `300 rps · 10분`을 통과했다. `350 rps`는 1분 측정을 통과했지만 10분 측정에서는 실패해 현재 확인된 지속 통과 기준은 `300 rps`다.
...
| 7 | [MID4-244 실패 경계 후속 재측정](mid4-244-rdb-failed-boundary-remeasure/README.md) | 부분 커버링 인덱스는 어디까지 효과가 있고 일반 조회는 300 rps보다 얼마나 더 처리할 수 있는가 | 데이터 몰림 조건은 275 rps가 변동이 있는 통과 지점이고 300 rps에서 반복 실패했다. 일반 데이터는 300 rps를 10분 유지했지만 350 rps는 10분 유지하지 못했다. 운영 인덱스 적용 여부는 결정하지 않았다. |
...
| 부분 커버링 인덱스 효과와 데이터 몰림·일반 조회의 처리량 경계 | [MID4-244 실패 경계 후속 재측정](mid4-244-rdb-failed-boundary-remeasure/README.md) |
...
| MID4-244 | MID4-227과 MID4-206에서 실패한 두 조건부터 재현하고 부분 커버링 인덱스와 처리량 경계를 확인 | 일반 `300 rps · 10분`은 현재 코드로 통과했다. 일반 350 rps는 1분 3회에는 통과했지만 10분에는 실패했다. 후보 인덱스를 적용한 데이터 몰림 조건은 250 rps까지 3회 통과하고 275 rps는 가운데 값 기준 통과, 300 rps는 반복 실패했다. 데이터 몰림 2배 조건은 200 rps부터 실패했다. 후보 인덱스는 현재 배포 상태가 아니다. |
...
- [API 실패 경계 상세 결과](mid4-244-rdb-failed-boundary-remeasure/api-boundary-results.md)
- [측정 조건과 재현 방법](mid4-244-rdb-failed-boundary-remeasure/measurement-and-reproduction.md)
...
| 카운터 테이블(counter table) | 댓글 수·조회 수처럼 자주 세는 값을 별도 메타데이터 테이블에 미리 저장하는 방식 |
| 역정규화(denormalization) | 읽기를 빠르게 하기 위해 원본에서 계산할 수 있는 값을 중복 저장하는 방식 |
| 노출 상태(visibility status) | 활동을 화면과 개수 계산에 포함할 수 있는지 활동 데이터에 저장한 값 |

## 다음에 확인할 내용

- 후보 인덱스 4개의 약 `479 MB` 저장 공간과 쓰기 비용을 확인한 뒤 운영 DB 반영 여부와 적용 대상을 결정한다. 현재 측정 결과만으로는 운영 적용을 완료한 것으로 보지 않는다.
- 이번 작업에서는 조회 SQL을 별도로 변경하지 않는다. 요청마다 정확한 개수를 다시 세기 때문에 데이터 몰림 조건에서도 약 `17.1만 개`, 데이터 몰림 2배 조건에서는 약 `34.1만 개`의 인덱스 항목을 확인했다.
- 데이터 몰림 조건은 `250 rps`까지 3회 통과, `275 rps`는 가운데 값 기준 통과지만 변동 있음, `300 rps`는 반복 실패로 기록한다. 데이터 몰림 2배 조건은 `200 rps`부터 실패했다.
- 일반 데이터는 `300 rps · 10분` 통과, `350 rps · 1분` 통과, `350 rps · 10분` 실패로 기록한다. `301~349 rps`의 10분 결과는 측정하지 않았으므로 정확한 최대값을 단정하지 않는다.
- 카운터 테이블은 자주 세는 값을 중복 저장하는 역정규화이므로 초기 구조에 미리 넣지 않는다. 개수 계산이 실제 병목이고 필요한 처리량이 현재 한계를 넘을 때 검토한다.
- MongoDB에 만든 조회용 데이터(Read Model)는 활동 내역 조회 전체를 RDB와 분리해 독립적으로 확장해야 할 때 검토한다. 개수 계산만 문제라면 더 작은 변경인 카운터 테이블을 먼저 비교한다.
- 서비스 공개 직후에는 실제 사용자 요청 기록이 충분하지 않으므로 RPS를 사용자 수로 환산하지 않는다. 운영 로그가 쌓인 뒤 최고 RPS와 p95·p99를 같은 기준으로 비교한다.
- 모든 결과는 로컬·단일 사용자·워밍업 캐시 조건이므로 운영 보장값으로 사용하지 않는다.
