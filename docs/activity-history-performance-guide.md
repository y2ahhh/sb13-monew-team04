...
- 일반 데이터에서 `300 rps`보다 높은 구간을 확인한 결과 `350 rps`는 1분 측정 3회를 통과했지만 10분 측정에서는 실패했다. 1분 측정에서 `375 rps`는 3회 중 1회, `400 rps`는 3회 중 2회가 실패했다. 따라서 현재 확인된 지속 통과 기준은 `300 rps`다.
