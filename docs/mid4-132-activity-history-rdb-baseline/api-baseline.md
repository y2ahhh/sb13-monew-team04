# 단일 사용자 활동내역 API 개선 전 결과

> [MID4-132 요약](README.md) · [활동내역 성능 문서 통합 안내서](../activity-history-performance-guide.md)

## 한눈에 보기

- 이 문서는 네 가지 활동내역을 한 번에 반환하는 API 전체의 개선 전 속도를 기록한다.
- `100k`, `1m` 테스트 데이터에서는 초당 20건을 처리했지만 `10m`에서는 요청 979건을 제때 시작하지 못했다.
- HTTP 오류는 없었지만 응답이 매우 느려졌으므로, 문제는 성공 여부가 아니라 처리 속도와 처리량이었다.
- 상세 수치는 이후 인덱스 적용 결과와 같은 조건으로 비교하는 기준값이다.
