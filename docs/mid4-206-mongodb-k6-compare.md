...
- `dev-sql-warn`은 같은 dev profile에서 `org.hibernate.SQL=warn`, `org.hibernate.orm.jdbc.batch=warn`, `org.hibernate.orm.jdbc.bind=off`를 애플리케이션 기동 인자로 override한 조건이다.
