...
후속 적용이 확정된 경우 전체 흐름은 다음과 같이 진행한다.

```text
1. 활동내역 4개 기능을 RDB로 구현한다.
2. 10만 / 100만 / 1000만 건 테스트 데이터를 준비한다.
...
7. 최적화 후에도 가장 병목이 큰 기능을 MongoDB 후속 적용 후보로 선정한다.
8. 해당 기능의 MongoDB Read Model을 설계한다.
...
13. 성능 개선과 운영 복잡도 대비 이득이 확인되면 MongoDB 적용 여부를 결정한다.
```
...
`status`는 activity가 노출되는지와 별개로 현재 상태 또는 숨김 사유를 표현한다. 기본 상태는 `ACTIVE`이며, 좋아요 취소는 `CANCELED`, 구독 해제는 `UNSUBSCRIBED`, 기사/댓글 삭제 또는 비공개 처리는 `TARGET_DELETED`, 사용자 삭제 또는 탈퇴는 `USER_DELETED`로 둔다.
