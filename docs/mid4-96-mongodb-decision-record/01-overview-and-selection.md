...
MongoDB 적용 대상은 아래 4개 활동내역 조회 기능 중 RDB 최적화 이후에도 성능이 가장 떨어지는 기능으로 선정한다.
...
최근 본 뉴스 기사 조회: p95 160ms
```

MongoDB 적용 대상은 `최근 좋아요한 댓글 조회`로 선정한다.
...
RDB 방식과 MongoDB Read Model 방식은 동일한 데이터 규모와 동일한 시나리오에서 비교한다.
...
가능하면 VU 기준뿐 아니라 RPS 기준도 함께 측정한다.
...
따라서 병목 여부는 다음을 함께 판단한다.

```text
병목 여부 = 단일 쿼리 시간 + 초당 요청 수 + 요청당 SQL 개수 + DB 리소스 사용률 + 커넥션 풀 대기
```
